...
for(i=t_c; i<N; i++){
	for(j=1; j<M; j++){
		k1[j]=v[i][j];
		l1[j]=-((v[i][j]-v[i][j-1])/fabs(x[i][j]-x[i][j-1]));
	}
	for(j=1; j<M; j++){
		k2[j]=v[i][j]+0.5*dt*l1[j];
		l2[j]=-(((v[i][j]+0.5*dt*l1[j])-(v[i][j-1]+0.5*dt*l1[j-1]))/fabs(((x[i][j]+0.5*dt*k1[j])-(x[i][j-1]+0.5*dt*k1[j-1]))));
	}
	for(j=1; j<M; j++){
		k3[j]=v[i][j]+0.5*dt*l2[j];
		l3[j]=-(((v[i][j]+0.5*dt*l2[j])-(v[i][j-1]+0.5*dt*l2[j-1]))/fabs((x[i][j]+0.5*dt*k2[j])-(x[i][j-1]+0.5*dt*k2[j-1])));
	}
	for(j=1; j<M; j++){
		k4[j]=v[i][j]+dt*l3[j];
		l4[j]=-(((v[i][j]+dt*l3[j])-(v[i][j-1]+dt*l3[j-1]))/fabs((x[i][j]+dt*k3[j])-(x[i][j-1]+dt*k3[j-1])));
	}
	for(j=1; j<M; j++){
...
output1=fopen("Trafic tau=0 posicions", "w");
...
output2=fopen("Trafic tau=0 velocitats", "w");
